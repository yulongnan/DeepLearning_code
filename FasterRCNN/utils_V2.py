...
class LossHistory_interrupt():  
    def __init__(self, log_dir, Flag_interrupt = False):   
        import datetime  
        curr_time = datetime.datetime.now() 
        time_str = datetime.datetime.strftime(curr_time,'%Y_%m_%d_%H_%M_%S') 
        self.log_dir    = log_dir 
        ## 修改部分 = 中断后连续处理
        if Flag_interrupt: #出现中断        
            files = os.listdir(self.log_dir)  
            for file in files:
                if file[:5] == 'loss_':
                    self.time_str = file[5:]    # self.time_str  
                    print(self.time_str) 
            self.save_path = os.path.join(self.log_dir, "loss_" + str(self.time_str))
                        
            rootpath1 = os.path.join( self.log_dir,'loss_'+ self.time_str, 'epoch_loss_'+ self.time_str + '.txt')
            self.losses = list( np.loadtxt(rootpath1, delimiter=',', usecols=(1)) )

            rootpath2 = os.path.join( self.log_dir,'loss_'+ self.time_str, 'epoch_val_loss_'+ self.time_str + '.txt')
            self.val_loss  =list( np.loadtxt(rootpath2, delimiter=',', usecols=(1)) )   
        else: ## 首次
            self.time_str   = time_str
            self.save_path  = os.path.join(self.log_dir, "loss_" + str(self.time_str))
            self.losses     = []
    ...
    def append_loss(self, loss, val_loss, epoch):
        ...
        self.val_loss.append(val_loss)
        with open(os.path.join(self.save_path, "epoch_loss_" + str(self.time_str) + ".txt"), 'a') as f:
            f.write( 'epoch'+ str(epoch).zfill(3) + ','+ str(loss) )      ##修改  f.write(str(loss))  
            f.write("\n")  
        with open(os.path.join(self.save_path, "epoch_val_loss_" + str(self.time_str) + ".txt"), 'a') as f:
            f.write( 'epoch'+ str(epoch).zfill(3) + ','+ str(val_loss) )  ##修改f.write(str(val_loss))
            f.write("\n")  
        self.loss_plot()   
